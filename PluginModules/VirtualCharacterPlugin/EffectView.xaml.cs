...
namespace VirtualCharacterPlugin
{
    /// <summary>
    ...
    public partial class EffectView :  UserControl
    {
        ...
        public void RefreshPostion()
        {
            try
            {
                ...
                var parendGrid = (Grid)this.Parent;
                {
                    ...
                    vm.ShowWidth = vm.ShowHeight = 340 * _showRatio;

                    {
                        //double hov = ((double)_showPoint.X / 500 * (parendGrid.ActualWidth - this.Width));

                        double hov = _showPoint.X * (parendGrid.ActualWidth / 340.0);
                        Thickness.Left = hov;
...

                    {
                        ...
                        double vec = _showPoint.Y * (parendGrid.ActualHeight / 340.0);
                        Thickness.Top = vec;
        ...
        private void EffectView_Loaded(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                string str =$@"pack://application:,,,/VirtualCharacterPlugin;Component/Resources/config.json";
                ...
                using (Stream stream = Application.GetResourceStream(new Uri(str, UriKind.RelativeOrAbsolute))?.Stream)
                {
                    using (StreamReader r = new StreamReader((Stream)stream, Encoding.UTF8))
                    {
                        ...
                        EffectConfig = HYWEffectConfig.EffectConfigTool.EffectConfigParseString(sCfgData, _dllPath, "VirtualCharacterPlugin_v1.0", _monitorIndex, 1);
